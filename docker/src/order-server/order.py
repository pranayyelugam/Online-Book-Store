...
def buyRequestForItem(itemNumber):
    queryResponse = requests.get(app.config.get('loadbalancer_uri') + '/@query@' + str(itemNumber)).json()
    if queryResponse:
        stock = int(queryResponse[0]["stock"]) - 1
        if (stock >= 0):
            try:
                updateResponse =  requests.get(app.config.get('loadbalancer_uri') + '/@reduceStock@'+ str(itemNumber))
